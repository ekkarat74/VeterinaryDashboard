...
const BreedModal = ({ isOpen, onClose, apiBaseUrl, token, onToast }) => {
    ...
    const fetchItems = async () => {
        ...
        } catch (err) { onToast('error', 'โหลดข้อมูลสายพันธุ์ไม่สำเร็จ'); }
    };
            ...
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-md flex flex-col max-h-[80vh] animate-in fade-in zoom-in-95">
                ...
                <div className="p-4 border-b border-slate-100 bg-slate-50">
                    <form onSubmit={handleAdd} className="flex gap-2">
                        <input type="text" placeholder="พิมพ์ชื่อสายพันธุ์ใหม่..." className="flex-1 p-2.5 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none" value={newItem} onChange={(e) => setNewItem(e.target.value)} />

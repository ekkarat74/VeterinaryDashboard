...
// 2. กำหนด Type สำหรับข้อมูลสีที่ได้รับจาก API
interface ColorItem {
    _id: string;
    name: string;
}

// 3. กำหนด Type สำหรับ Props ของ Component
...
const ColorModal: React.FC<ColorModalProps> = ({ isOpen, onClose, apiBaseUrl, token, onToast }) => {
    ...
    const fetchItems = async (): Promise<void> => {
        try {
            ...
            if (res.ok) {
                const data: ColorItem[] = await res.json();
                setItems(data);
            }
        } catch (err) { 
            onToast('error', 'โหลดข้อมูลสีไม่สำเร็จ'); 
        }
    };

    // 5. ระบุ Type ให้กับ Event ของ Form
    const handleAdd = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                // ✨ ป้องกันแอปพังจากการแปลงหน้า 404 HTML เป็น JSON
    ...
    // 6. ระบุ Type ของ parameter id ให้เป็น string

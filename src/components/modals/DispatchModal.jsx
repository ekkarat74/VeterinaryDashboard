            ...
            <div className="bg-white w-full max-w-5xl rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden ring-1 ring-slate-900/5">
                ...
                <div className="p-4 bg-white border-t border-slate-200 flex flex-col sm:flex-row gap-3 justify-between items-center shrink-0">
                    {initialData && onDelete ? (
                        <button onClick={() => onDelete(initialData._id)} className="text-red-500 hover:text-red-700 text-sm font-semibold flex items-center gap-1 px-3 py-2 hover:bg-red-50 rounded-lg transition-colors">
                            <Trash2 className="w-4 h-4" /> ลบรายการ
                        </button>
                    ) : <div></div>}

                    <div className="flex w-full sm:w-auto gap-3">
                        <button onClick={onClose} className="flex-1 sm:flex-none px-6 py-2.5 border border-slate-300 text-slate-700 font-bold rounded-xl hover:bg-slate-50 transition-colors">
                            ยกเลิก
                        </button>
                        <button onClick={handleSaveLocal} className="flex-1 sm:flex-none px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-lg shadow-indigo-200 transition-all transform hover:-translate-y-0.5">
                            {initialData ? 'บันทึกแก้ไข' : 'บันทึกข้อมูล'}
                        </button>
                        <button onClick={handleSendLine} className="flex-1 sm:flex-none px-6 py-2.5 bg-[#06C755] hover:bg-[#05b64d] text-white font-bold rounded-xl shadow-lg shadow-green-200 flex items-center justify-center gap-2 transition-all transform hover:-translate-y-0.5">

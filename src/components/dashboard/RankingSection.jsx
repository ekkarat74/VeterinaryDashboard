...
const RankingSection = ({
  type = "all", // "all", "table", หรือ "deepdive"
  rankingYear,
  setRankingYear,
  rankingMonth,
  setRankingMonth,
        ...
        <div className="bg-white rounded-2xl shadow-lg shadow-slate-200/60 border border-slate-100 flex flex-col flex-1 overflow-hidden h-full min-h-[500px] max-h-[600px]">
          <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-white sticky top-0 z-20 shrink-0 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-md font-bold text-slate-800 leading-tight">เจาะลึก 5 อันดับแรก</h2>
                <p className="text-xs text-slate-500">แยกตามประเภทบริการ และเขตพื้นที่ (เลื่อนลงเพื่อดูเพิ่มเติม)</p>
              </div>
            </div>
          </div>
          
          {/* ใช้ flex-col และ overflow-y-auto เพื่อให้เลื่อนแนวตั้งได้ โดยไม่ทะลุกรอบ */}
              ...
              rankingNestedStats.map((unit, index) => (
                <div key={index} className="bg-white rounded-xl p-4 border border-slate-200 shadow-sm shrink-0 flex flex-col hover:shadow-md transition-shadow">
                  ...
                  <div className="space-y-1.5 pt-2 border-t border-slate-50 flex-1">
                    <p className="text-[10px] text-slate-400 font-bold uppercase mb-1.5 pl-1">พื้นที่ให้บริการสูงสุด (Top 5 Areas)</p>
                    {unit.topDistricts && unit.topDistricts.length > 0 ? (
                          ...
                          <div key={dIndex} className="flex flex-col py-1.5 px-2 rounded hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-0 relative overflow-hidden group">
                            ...
                            <div className="relative z-10 flex justify-between items-center mb-1">
                              <div className="flex items-center gap-1.5">
                                <span className="w-3 text-[9px] text-slate-400 font-mono">{dIndex + 1}.</span>
                                <span className="text-xs text-slate-700 font-bold truncate max-w-[120px]" title={dist.name}>{dist.name}</span>
                              </div>
                              <div className="flex items-center gap-1.5 shrink-0">
                                <span className="text-[8px] text-slate-400">{distPercent}%</span>
                                <span className="text-[9px] font-bold text-indigo-600 bg-white shadow-[0_1px_2px_rgba(0,0,0,0.05)] border border-indigo-50/50 px-1.5 py-0.5 rounded-md">
                      ...
                      <div className="text-center text-xs text-slate-400 py-4">- ไม่ระบุเขต -</div>
                    )}
                  </div>
                </div>
              ))

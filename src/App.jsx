...
// 13. DispatchHistoryModal (✅ หน้าต่างดูประวัติการออกหน่วยแบบรายการ)
const DispatchHistoryModal = ({ isOpen, onClose, data, onDelete, isLoading }) => {
    if (!isOpen) return null;
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[3100] flex items-center justify-center p-4 animate-in fade-in">
            <div className="bg-white rounded-2xl w-full max-w-4xl shadow-2xl flex flex-col max-h-[85vh] overflow-hidden">
                <div className="bg-indigo-600 px-6 py-4 flex justify-between items-center text-white shrink-0">
                    <h3 className="text-lg font-bold flex items-center gap-2">
                        <History className="w-5 h-5" /> ประวัติการออกหน่วย (Dispatch List)
                    </h3>
                    <button onClick={onClose}><X className="w-5 h-5" /></button>
                </div>
                    ...
                    <table className="w-full text-sm text-left">
                        <thead className="bg-slate-50 text-slate-500 font-bold sticky top-0 shadow-sm z-10">
                            <tr>
                                ...
                                <th className="p-4">ทีมปฏิบัติงาน</th>
                                <th className="p-4 text-center">จัดการ</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                            {isLoading ? (
                                <tr><td colSpan="6" className="p-8 text-center">กำลังโหลด...</td></tr>
                            ) : data.length === 0 ? (
                                ...
                                data.map((item) => (
                                    <tr key={item._id} className="hover:bg-slate-50 transition-colors">
                                        ...
                                        <td className="p-3 text-indigo-600 font-bold font-mono">{item.time}</td>
                                        <td className="p-3">
                                            <span className={`px-2 py-1 rounded text-[10px] font-bold border ${
                                                ...
                                                {item.type === 'sterilization' ? 'ทำหมัน' : 'วัคซีน+ชิป'}
                                            </span>
                                        </td>
                                        <td className="p-3">
                                            <div className="font-medium text-slate-800">{item.location}</div>
                                            {item.note && <div className="text-[10px] text-slate-400 truncate max-w-[150px]">{item.note}</div>}
                                        </td>
                                        <td className="p-3 text-xs text-slate-600 max-w-[150px] truncate" title={item.team}>
                                            {item.team || '-'}
                                        </td>
                                        <td className="p-3 text-center">
                                            {onDelete && (
...
    const [isDispatchHistoryOpen, setIsDispatchHistoryOpen] = useState(false);
    const [dispatchEvents, setDispatchEvents] = useState([]); // เปลี่ยนจาก Mockup เป็น array ว่างรอ fetch
...
    const handleSaveDispatchEvent = async (payload) => {
        try {
            ...
            if (res.ok) {
                addToast('success', 'บันทึกแผนงานเรียบร้อย');
                fetchDispatchEvents(); // รีโหลดข้อมูล
                // setIsCalendarOpen(true); // เปิดปฏิทินทันทีถ้าต้องการ
            } else {
                addToast('error', 'บันทึกไม่สำเร็จ');
            }
        } catch (error) {
            addToast('error', 'เชื่อมต่อ Server ไม่ได้');
        }
    };
    ...
    const handleDeleteDispatch = async (id) => {
        ...
        try {
            const res = await fetch(`${BASE_URL}/api/dispatch/${id}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${user?.token}` }
            });
            if(res.ok) {
                addToast('success', 'ลบรายการสำเร็จ');
                setDispatchEvents(prev => prev.filter(e => e._id !== id));
            }
        } catch(error) {
    ...
    const fetchDispatchEvents = async () => {
        try {
            setIsDispatchLoading(true);
            const res = await fetch(`${BASE_URL}/api/dispatch`);
            const data = await res.json();
            // Map ข้อมูลให้ตรงกับ format ที่ Calendar ใช้งาน
            const formattedData = Array.isArray(data) ? data.map(d => ({
                ...d,
                // calendar ต้องการ key เหล่านี้
                title: d.type === 'sterilization' ? 'ทำหมัน' : 'วัคซีน', 
                team: d.staff?.vets ? d.staff.vets.join(', ') : d.team
            })) : [];
            setDispatchEvents(formattedData);
        } catch (error) {
...
            <AddOutbreakModal isOpen={isOutbreakModalOpen} onClose={() => setIsOutbreakModalOpen(false)} onSave={handleAddOutbreak} onUpdate={handleUpdateOutbreak} initialData={editingOutbreak} onToast={addToast} />
            ...
            <DispatchHistoryModal 
                ...
                data={dispatchEvents}
                onDelete={canEdit ? handleDeleteDispatch : null}
...
                                <button onClick={() => setIsCsvModalOpen(true)} className="p-2 text-slate-600 hover:text-slate-900 hover:bg-slate-100 rounded-full" title="CSV Import/Export"><Download className="w-5 h-5" /></button>
                                ...
                                className="p-2 text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-full border border-indigo-200 transition-all"
                                title="ดูบันทึกรายการออกหน่วย (List View)"
                            ></button>
                                <button 
